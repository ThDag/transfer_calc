...
- Twin currency values (Amount In, Amount Out) share one bordered box, stacked top over bottom and split by an internal rule, never two separate fields that merely sit near each other
- No outer card — the black page itself is the canvas; borders exist only where an individual element earns one
- Every ledger row (Service Comm, Our Comm, Comm Main) shows both its USD and AED value side by side, each independently tap-to-copy
...
### Primary
- **Sage** (#87a96b): a calm, muted green reserved entirely for the Our Comm row's label and value text — the owner's own earnings. Never used for chrome, borders, Amount Out, Comm Main, or Service Comm.
...
Information is arranged to make the calculation legible, not just correct, and the page's vertical order **is** the calculation's own dependency order: Amount In (the principal the operator sets) → Service/Our rates → the commission ledger → Amount Out, the bottom-line result the ledger produces → the exchange rate override → Export Summary. Amount Out sits above the exchange rate not because it matters less, but because the exchange rate isn't part of the calculation's flow at all — it's a rare manual override, so it belongs at the very bottom with the export action, never interrupting the principal-to-result chain partway through.

Amount In and Amount Out are each one bordered box holding USD and AED as two sibling faces of the same value, **stacked top over bottom and split by one internal horizontal rule** — not side by side — rather than drawn as separate fields. Editing either face recomputes the other, and the shared border is what tells the reader they're one thing, not two; stacking (rather than splitting the box's width in half) gives each face the full width to stay legible at a size close to a real primary input. Below Amount In, two adjustable rate inputs (Service Rate %, Our Rate %) sit side by side, captioned once ("Both rates below apply to Amount In above" — no currency named, since Amount In is one value, not an AED-specific quantity) so the base every percentage applies to is never ambiguous. Below the divider, Service Comm and Our Comm are drawn as an explicit addition ledger — a literal "+" between the two rows, a rule, then Comm Main labeled with its own combined percentage ("X% of Amount In") as the sum, styled at the same size as its two parts. Every ledger row now shows **both** its USD and AED faces side by side, each independently tappable to copy — the commission figures get the same twin treatment as the principal and the result, just inline in a row instead of stacked in a bordered box. Nothing about any of these relationships — twin currencies, or the commission sum — is left for the reader to infer from position alone.
...
### Twin Pair Field (Amount In, Amount Out)
...
- **One label, not two:** a single uppercase label ("Amount In" / "Amount Out") sits above the whole bordered group; neither segment gets its own label, because they're not two fields, they're two faces of one field
...
- **Style:** Service Comm, Our Comm, and Comm Main each show **both** a USD and an AED value, side by side at the end of their row (label, then the USD figure, then the AED figure) — neither has a separate copy button; each value IS the control, rendered as an unstyled-at-rest `<button>` (no border, no background, `font: inherit`) so it looks exactly like the plain text it replaces until interacted with
- **Independence:** the two values in a row are separately tappable and copy independently — tapping the USD figure copies only "$X"; tapping the AED figure copies only "AED Y". They are never combined into one copyable string.
- **Confirmation:** on tap, that one value's own background and text invert (Sage background, Void Black text) for about a second, then revert — deliberately **not** just a color change, because Our Comm's text is already Sage at rest; an icon swap or color-only flash would be invisible or redundant there. The invert reads clearly regardless of the row's resting color, so one confirmation mechanism covers every value in every row.
- **Why not a button next to it:** a value that's directly tappable removes a whole extra element per row; the Twin Pair Field's inline mini-copy buttons exist because those rows already have room and a clear boundary (the segment) to anchor a button to — the ledger's plain rows don't, so each value absorbs its own action instead.

### Exchange Rate Field
- **Position:** bottom of the page, directly above the Export Summary button — deliberately last, alongside the other action a user rarely touches, rather than up near Amount In where it would compete with the primary inputs for attention
...
- **Why it's quiet, and why it's last:** it's an edge-case override (the AED peg almost never actually changes), not a step in the calculation flow — Amount In flows straight through the rates and the ledger to Amount Out with nothing interrupting it; the exchange rate sits below all of that, grouped with the export action instead of the primary flow.
...
- **What it copies:** every field as one organized block of text, in the page's own reading order (USD/AED principal, rates, the commission ledger, then Amount Out as the bottom line) — see PRODUCT.md for the exact format
...
### Commission Ledger
- **Style:** a plain row (Service Comm), a centered gray "+" glyph, a second row in Sage (Our Comm), a 2px white rule, then Comm Main at the **same size and weight as the two rows above it** — the whole group reads top to bottom as one addition, and the sum doesn't get to be visually louder than its own parts just for being the total. Every row carries both a USD and an AED value (see Ledger Value).
- **Why this shape:** the Shown-Sum Rule — the reader should never have to trust that Comm Main equals the two rows above it; the page proves it, in both currencies (Service Comm's USD + Our Comm's USD sums to Comm Main's USD exactly, same as the AED side, since the USD figures are always derived the same way — never computed independently). Size is not part of that proof: Comm Main is distinguished only by position (last, after the rule), never by being drawn bigger.
...
- **Do** order the primary flow (Amount In, rates, ledger, Amount Out) to match the calculation's own dependency chain, and keep rare manual overrides like the exchange rate at the very bottom, grouped with the export action rather than interrupting that flow.
