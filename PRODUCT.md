...
- **Service Comm, Our Comm, and Comm Main copy by tapping the value itself** — no separate button. Each value is rendered as a `<button>` (not a `<span>`), and tapping it copies its displayed text (already includes the "AED" prefix). Confirmation is a background/text invert flash (`.copied`: accent background, black text) rather than an icon swap, chosen specifically because Our Comm's text is already accent-colored at rest — a color-only confirmation would be invisible there, so the flash inverts foreground/background instead, which reads clearly on all three rows regardless of resting color. Plus the main Export Summary button copies everything at once (not on Service Comm/Our Comm/Comm Main individually vs. the export — both exist, for different purposes).
...
- Must work fully offline once installed (service worker pre-caches all three files). The cache name in `sw.js` gets bumped on every meaningful change to `index.html` so an already-installed phone picks up the update on next launch — currently `havcal-v15`.
